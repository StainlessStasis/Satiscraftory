...
public class MergerBlock extends AbstractDirectionalFactoryBlock {
    ...
    private static final MapCodec<MergerBlock> CODEC = simpleCodec(MergerBlock::new);
    ...
    public @Nullable BlockEntity newBlockEntity(@NonNull BlockPos pos, @NonNull BlockState state) {
        return new MergerBlockEntity(ManifoldBlockEntities.MERGER.get(), pos, state);
    }

    @Override
    protected void notifyNeighborChanged(BlockEntity blockEntity, ServerLevel level) {
        if (blockEntity instanceof MergerBlockEntity mergerBE) mergerBE.onNeighborChanged();
    }
    ...
    protected void affectNeighborsAfterRemoval(@NonNull BlockState state, @NonNull ServerLevel level, @NonNull BlockPos pos, boolean movedByPiston) {
        ...
        FactoryNetwork.get(level).removeMerger(GlobalPos.of(level.dimension(), pos));
    }
